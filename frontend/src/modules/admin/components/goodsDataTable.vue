...
export default defineComponent({
  ...
  setup() {
    ...
    const pageNumber = ref<any>(queryParams.value.pageNumber);
    const pageSize = ref<any>(queryParams.value.pageSize);
    ...
    const getDataForTable = async () => {
      ...
      timerForDataTebleSearch.value = setTimeout(async () => {
        await fetchGoods(pageSize.value, pageNumber.value).then(() => {
          pageCount.value = Math.ceil(totalGoods.value / pageSize.value);
        });
      }, 500);
    };

    onMounted(() => {
      queryParams.value = getURIQueryParams(route.query, queryParamsConfig);
      pageNumber.value = queryParams.value.pageNumber;
      pageSize.value = queryParams.value.pageSize;
      orderBy.value = queryParams.value.orderBy;
      getDataForTable();
    });
